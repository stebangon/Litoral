...
import imgCotizar from '../assets/Recursos - LITORAL-89.svg'; // with import
...
export default class Lightbox extends React.Component {
  ...
  createTransform = (x, y, zoom, rotate) =>
    `translate3d(
        ${x}px,${y}px,0px) scale(${zoom}) rotate(${rotate}deg)`;
  stopSideEffect = (e) => e.stopPropagation();
  getCurrentImage = (s, p) => {
    if (!s.multi) return p.image ?? '';
    return p.images[s.current]?.src ?? p.images?.[s.current] ?? '';
  };
  getCurrentTitle = (s, p) => {
    if (!s.multi) return p.title ?? '';
    return p.images?.[s.current]?.title ?? '';
  };
  getCurrentSubTitle = (s, p) => {
    if (!s.multi) return p.subtitle ?? '';
    return p.images?.[s.current]?.subtitle ?? '';
  };
  ...
  shockZoom = (e) => {
    ...
    const _z =
      zoomStep < 1
        ? Math.ceil(doubleClickZoom / zoomStep)
        : zoomStep * zoomStep;
    const _xy = getXY(e);
  ...
  render() {
    ...
    let subtitle = this.getCurrentSubTitle(this.state, this.props);
    if (!image) {
